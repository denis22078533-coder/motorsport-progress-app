...
def web_search_ddg(query: str, max_results: int = 5) -> list[dict]:
    """Ищет в интернете через DuckDuckGo Instant Answer API."""
    results = []
    try:
        encoded = urllib.parse.quote(query)
        url = f"https://api.duckduckgo.com/?q={encoded}&format=json&no_html=1&skip_disambig=1"
        req = urllib.request.Request(url, headers={
            "User-Agent": "Mozilla/5.0 (compatible; MuraveyBot/1.0)",
...
def web_search_bing_text(query: str, max_results: int = 5) -> list[dict]:
    """Ищет текстовые результаты через Bing (scraping)."""
    results = []
    try:
        encoded = urllib.parse.quote(query)
        url = f"https://www.bing.com/search?q={encoded}&count=10&setlang=ru"
        req = urllib.request.Request(url, headers={
        ...
        # Извлекаем заголовки и сниппеты из Bing SERP
        titles = re.findall(r'<h2[^>]*><a[^>]*href="([^"]+)"[^>]*>(.*?)</a></h2>', html)
        snippets = re.findall(r'<p class="b_algoSlug[^"]*">(.*?)</p>', html)
        ...
        for i, (link, title) in enumerate(titles[:max_results]):
            ...
            if clean_title and link.startswith("http"):
                results.append({"title": clean_title, "snippet": snippet, "url": link})
    except Exception as e:
        print(f"[web-search] bing text error: {e}")
    return results[:max_results]


def web_search_brave(query: str, api_key: str, max_results: int = 5) -> list[dict]:
    """Ищет через Brave Search API (если есть ключ)."""
    results = []
    try:
        encoded = urllib.parse.quote(query)
        url = f"https://api.search.brave.com/res/v1/web/search?q={encoded}&count={max_results}&search_lang=ru"
        req = urllib.request.Request(url, headers={
            "Accept": "application/json",
...
def web_search(query: str, max_results: int = 5) -> list[dict]:
    """Многоуровневый поиск в интернете: Brave API → DDG → Bing scraping."""
    import os
    ...
    if brave_key:
        results = web_search_brave(query, brave_key, max_results)
        if results:
            print(f"[web-search] brave: {len(results)} results")
            return results

    results = web_search_ddg(query, max_results)
    if results:
        print(f"[web-search] ddg: {len(results)} results")
        return results

    results = web_search_bing_text(query, max_results)
    print(f"[web-search] bing: {len(results)} results")
    return results
...
def search_product_image(article: str, name: str) -> str | None:
    """Ищет реальное фото товара: Google CSE → Bing → Яндекс → Openverse → AI-генерация"""
    import os
...
def handler(event: dict, context) -> dict:
    """Проксирует запросы к OpenAI/Claude API. Поддерживает: search_image, web_search."""
    ...
    # ── Поиск в интернете ────────────────────────────────────────────────────
    if body.get("__action__") == "web_search":
        query = body.get("query", "").strip()
        max_results = int(body.get("max_results", 5))
        if not query:
            return {"statusCode": 400, "headers": cors_headers, "body": json.dumps({"error": "query required"})}
        results = web_search(query, max_results)
        return {"statusCode": 200, "headers": cors_headers, "body": json.dumps({"results": results, "query": query})}
